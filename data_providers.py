...
class QuickDrawImageDataProvider(DataProvider):
    """Data provider for QuickDraw images."""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, flatten=False, one_hot=False,
                 train_ratio=0.8, valid_ratio=0.08, test_ratio=0.12,
                 num_classes_use=10):
        """Create a new QuickDraw data provider object.

        Args:
            which_set: One of 'train', 'valid' or 'eval'. Determines which
                portion of the QuickDraw data this object should provide.
            batch_size (int): Number of data points to include in each batch.
            max_num_batches (int): Maximum number of batches to iterate over
                in an epoch. If `max_num_batches * batch_size > num_data` then
                only as many batches as the data can be split into will be
        ...
        assert (train_ratio + valid_ratio + test_ratio) == 1.0, (
            'Expected the sum of ratios to be 1.0. '
            'Got train: {0}, valid: {1}, test: {2}'.format(str(train_ratio),
                                                           str(valid_ratio),
                                                           str(test_ratio))
        )
        self.one_hot = one_hot
        ...
        data_file_name = os.path.join('quickdraw_data', 'image_data_100.npy')
        classes_file_name = os.path.join('quickdraw_data', 'image_data_100_classes.npy')
        data = np.load(data_file_name)
        class_names = np.load(classes_file_name)
        self.num_classes = len(class_names)

        inputs = np.empty((0, 28, 28))
        targets = np.empty((0))
        num_items_in_class = 25000
        for class_index in range(min(self.num_classes, num_classes_use)):
            index_1 = class_index * num_items_in_class
            index_2 = index_1 + int(num_items_in_class * train_ratio)
            index_3 = index_1 + int(num_items_in_class * (train_ratio + valid_ratio))
            index_4 = index_1 + num_items_in_class

            if which_set=='train':
                chosen_data = data[index_1:index_2]
                class_targets = [class_index] * int(num_items_in_class * train_ratio)
            elif which_set=='valid':
                chosen_data = data[index_2:index_3]
                class_targets = [class_index] * int(num_items_in_class * valid_ratio)
            else:
                chosen_data = data[index_3:index_4]
                class_targets = [class_index] * int(num_items_in_class * test_ratio)

            inputs = np.append(inputs, chosen_data, axis=0)
            targets = np.append(targets, class_targets, axis=0)
        ...
        # pass the loaded data to the parent class __init__
        super(QuickDrawImageDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(QuickDrawImageDataProvider, self).next()
        if self.one_hot:
            return inputs_batch, self.to_one_of_k(targets_batch)
        else:
            return inputs_batch, targets_batch
...
class QuickDrawStrokeDataProvider(DataProvider):
    """Data provider for QuickDraw images."""
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_file_name = os.path.join('quickdraw_data', 'strokes_data_100.npy')
        classes_file_name = os.path.join('quickdraw_data', 'strokes_data_100_classes.npy')
        data = np.load(data_file_name)
        class_names = np.load(classes_file_name)
        self.num_classes = len(class_names)

        inputs = np.empty((0, 70, 3))
        targets = np.empty((0))
        num_items_in_class = 25000
        for class_index in range(min(self.num_classes, num_classes_use)):
            index_1 = class_index * num_items_in_class
        ...
        # pass the loaded data to the parent class __init__
        super(QuickDrawStrokeDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(QuickDrawStrokeDataProvider, self).next()
        if self.one_hot:
            return inputs_batch, self.to_one_of_k(targets_batch)
        else:
            return inputs_batch, targets_batch
...
class QuickDrawCombinedDataProvider(DataProvider):
    """Data provider for QuickDraw images."""
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_file_name_st = os.path.join('quickdraw_data', 'strokes_data_100.npy')
        classes_file_name = os.path.join('quickdraw_data', 'strokes_data_100_classes.npy')
        data_st = np.load(data_file_name_st)
        class_names = np.load(classes_file_name)

        data_file_name_im = os.path.join('quickdraw_data', 'image_data_100.npy')
        data_im = np.load(data_file_name_im)
        ...
        for class_index in range(min(self.num_classes, num_classes_use)):
            ...
            if which_set=='train':
                chosen_data_st = data_st[index_1:index_2]
                chosen_data_im = data_im[index_1:index_2]
                class_targets = [class_index] * int(num_items_in_class * train_ratio)
            elif which_set=='valid':
                chosen_data_st = data_st[index_2:index_3]
                chosen_data_im = data_im[index_2:index_3]
                class_targets = [class_index] * int(num_items_in_class * valid_ratio)
            else:
                chosen_data_st = data_st[index_3:index_4]
                chosen_data_im = data_im[index_3:index_4]
                class_targets = [class_index] * int(num_items_in_class * test_ratio)

            chosen_data_im = chosen_data_im.astype(np.float32)
            if flatten:
                chosen_data_im = np.reshape(chosen_data_im, newshape=(-1, 28*28))
            else:
                chosen_data_im = np.expand_dims(chosen_data_im, axis=3)
                chosen_data_im = chosen_data_im / 255.0

            # first stroke sequence data, then image data
            for ii in range(chosen_data_st.shape[0]):
                inputs.append([chosen_data_st[ii], chosen_data_im[ii]])
        ...
        # pass the loaded data to the parent class __init__
        super(QuickDrawCombinedDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(QuickDrawCombinedDataProvider, self).next()
        if self.one_hot:
            return inputs_batch, self.to_one_of_k(targets_batch)
        else:
            return inputs_batch, targets_batch
