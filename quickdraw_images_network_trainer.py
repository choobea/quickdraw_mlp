...
if network_name == 'fcn':
    ...
    if rotate:
        experiment_name = "exp{}_{}_{}_layers_{}_filter_augm".format(experiment_prefix, network_name,
    ...
    experiment_name = "exp{}_{}".format(experiment_prefix, network_name)
...
if network_name == 'convn':
    network_name = 'VGG_classifier' #'VGG_classifier'
elif network_name == "densenet":
    network_name = 'DenseNetClassifier'
elif network_name == "resnet":
    network_name = 'ResNetClassifier'
else:
    network_name = 'FCCClassifier'

if network_name == 'FCCClassifier':
    train_data = QuickDrawImageDataProvider(which_set="train", batch_size=batch_size, rng=rng, flatten=True, num_classes_use=num_classes_use)
    val_data = QuickDrawImageDataProvider(which_set="valid", batch_size=batch_size, rng=rng, flatten=True, num_classes_use=num_classes_use)
    test_data = QuickDrawImageDataProvider(which_set="test", batch_size=batch_size, rng=rng, flatten=True, num_classes_use=num_classes_use)
else:
    train_data = QuickDrawImageDataProvider(which_set="train", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
    val_data = QuickDrawImageDataProvider(which_set="valid", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
    test_data = QuickDrawImageDataProvider(which_set="test", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
...
if network_name == 'FCCClassifier':
    ...
    data_inputs = tf.placeholder(tf.float32, [batch_size, train_data.inputs.shape[1], train_data.inputs.shape[2], train_data.inputs.shape[3]], 'data-inputs')
...
classifier_network = ClassifierNetworkGraph(input_x=data_inputs, target_placeholder=data_targets,
                                            ...
                                            inner_layer_depth=inner_layer_depth, filter_size=filter_size)  # initialize our computational graph
