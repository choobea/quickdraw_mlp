...
if bidirectional:
    if len(conv_rnn_sizes) == 0:
        experiment_name = "exp{}_{}_{}_layers_bidirectional_{}_dropout".format(experiment_prefix, rnn_cell_type,
                                                     len(layer_stage_sizes), dropout_rate_value)
    else:
        experiment_name = "exp{}_{}_{}_layers_{}_rnnconv_bidirectional_{}_dropout".format(experiment_prefix, rnn_cell_type,
                                                     len(layer_stage_sizes), len(conv_rnn_sizes), dropout_rate_value)
else:
    experiment_name = "exp{}_{}_{}_layers_{}_dropout".format(experiment_prefix, rnn_cell_type,
                                                 len(layer_stage_sizes), dropout_rate_value)
if network_name == "FCNConvSeqClassifier":
    experiment_name = "exp{}_{}_layers_{}_rnnconv_{}_dropout_fcnconvseq".format(experiment_prefix,
                                                 len(layer_stage_sizes), len(conv_rnn_sizes), dropout_rate_value)
...
train_data = QuickDrawStrokeDataProvider(which_set="train", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
val_data = QuickDrawStrokeDataProvider(which_set="valid", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
test_data = QuickDrawStrokeDataProvider(which_set="test", batch_size=batch_size, rng=rng, num_classes_use=num_classes_use)
...
# Placeholder setup
data_inputs = tf.placeholder(tf.float32, [batch_size, train_data.inputs.shape[1], train_data.inputs.shape[2]], 'data-inputs')
...
# we would like to have a recurrent network here
        ...
        for e in range(start_epoch, epochs):
            ...
            with tqdm.tqdm(total=total_val_batches) as pbar_val:
                for batch_idx, (x_batch, y_batch) in enumerate(val_data):
                    lengths_strokes = np.array([x.shape[0] for x in x_batch], np.int32)
