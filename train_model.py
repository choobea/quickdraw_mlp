...
def get_input_fn(mode, tfrecord_pattern, batch_size):
  ...
  """
    ...
    def _parse_tfexample_fn(example_proto, mode):
        ...
        feature_to_type = {'ink': tf.VarLenFeature(dtype=tf.float32),
                           'shape': tf.FixedLenFeature([2],
                           dtype=tf.int64)}
      ...
      # to the list of feature_columns to be read.
            ...
            feature_to_type['class_index'] = tf.FixedLenFeature([1],
                    dtype=tf.int64)
        ...
        parsed_features['ink'] = \
            tf.sparse_tensor_to_dense(parsed_features['ink'])
    ...
    # Preprocesses 10 files concurrently and interleaves records from each file.
        ...
        dataset = dataset.map(functools.partial(_parse_tfexample_fn,
                              mode=mode), num_parallel_calls=10)
...
def model_fn(
  ...
  """
    ...
    # lengths will be [batch_size]
        ...
        lengths = tf.squeeze(tf.slice(shapes, begin=[0, 0],
    ...
    def _add_conv_layers(inks, lengths):
        ...
        for i in range(len(params.num_conv)):
            ...
            if params.batch_norm:
                convolved_input = \
                    tf.layers.batch_normalization(convolved_input,
      ...
      # Add dropout layer if enabled and not first convolution layer.
            ...
            if i > 0 and params.dropout:
                convolved_input = tf.layers.dropout(convolved_input,
                        rate=params.dropout, training=mode
    ...
    def _add_regular_rnn_layers(convolved, lengths):
        ...
        cells_fw = [cell(params.num_nodes) for _ in
                    range(params.num_layers)]
        cells_bw = [cell(params.num_nodes) for _ in
                    range(params.num_layers)]
    ...
    # Convolutions output [B, L, Ch], while CudnnLSTM is time-major.
        ...
        lstm = \
            tf.contrib.cudnn_rnn.CudnnLSTM(num_layers=params.num_layers,
                num_units=params.num_nodes,
                dropout=(params.dropout if mode
                == tf.estimator.ModeKeys.TRAIN else 0.0),
    ...
    # the number of nodes in the last layer.
        ...
        mask = tf.tile(tf.expand_dims(tf.sequence_mask(lengths,
                       tf.shape(outputs)[1]), 2), [1, 1,
                       tf.shape(outputs)[2]])
  ...
  # Add the optimizer.
    ...
    train_op = tf.contrib.layers.optimize_loss(  # some gradient clipping stabilizes training in the beginning.
        loss=cross_entropy,
  ...
  # Compute current predictions.
    ...
    return tf.estimator.EstimatorSpec(mode=mode,
            ...
            loss=cross_entropy, train_op=train_op,
...
def create_estimator_and_specs(run_config):
    ...
    train_spec = \
        tf.estimator.TrainSpec(input_fn=get_input_fn(mode=tf.estimator.ModeKeys.TRAIN,
                               tfrecord_pattern=FLAGS.training_data,
                               batch_size=FLAGS.batch_size),
                               max_steps=FLAGS.steps)

    eval_spec = \
        tf.estimator.EvalSpec(input_fn=get_input_fn(mode=tf.estimator.ModeKeys.EVAL,
...
def main(unused_args):
    (estimator, train_spec, eval_spec) = \
        create_estimator_and_specs(run_config=tf.estimator.RunConfig(model_dir=FLAGS.model_dir,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_conv', type=str, default='[48, 64, 96]',
                        help='Number of conv layers along with number of filters per layer.'
                        )
    parser.add_argument('--conv_len', type=str, default='[5, 5, 3]',
    ...
    parser.add_argument('--gradient_clipping_norm', type=float,
                        default=9.0,
    ...
    parser.add_argument('--self_test', type='bool', default='False',
                        help='Whether to enable batch normalization or not.'
                        )
